...
import numpy as np
import algorithms.maddpg.tf_util as U
from tensorflow.python.ops import math_ops
from gym.spaces.multi_discrete import MultiDiscrete
#from algorithms.maddpg.multi_discrete import MultiDiscrete
...
class MultiCategoricalPd(Pd):
    ...
    def flatparam(self):
        return self.flat
    def mode(self):
        return self.low + tf.cast(tf.stack([p.mode() for p in self.categoricals], axis=-1), tf.float32)
    def logp(self, x):
    ...
    def sample(self):
        return self.low + tf.cast(tf.stack([p.sample() for p in self.categoricals], axis=-1), tf.float32)
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        ac_space.low = np.array([x[0] for x in ac_space.nvec])
        ac_space.high = np.array([x[1] for x in ac_space.nvec])
