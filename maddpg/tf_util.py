...
# ================================================================
'''
def load_state_part(fname,agent_name):
    """Load all the variables to the current session from the location <fname>"""
    variables = slim.get_variables_to_restore() 
    variables_to_restore_agent = [v for v in variables if v.name.split('/')[0] == agent_name]

    saver_agent= tf.train.Saver(variables_to_restore_agent)
    saver_agent.restore(get_session(), fname)
    return saver_agent
'''
def load_state(fname, saver=None):
    ...
    saver.restore(get_session(), os.path.abspath(fname))
    return saver
...
def save_state(fname, saver=None):
    ...
    if saver is None:
        saver = tf.train.Saver()
    saver.save(get_session(), os.path.abspath(fname))
    return saver
